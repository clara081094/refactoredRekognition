...
class FoggyCam(object):
    ...
    def initialize_user(self):
        ...
        for bucket in user_object['updated_buckets']:
            ...
            if bucket_id.startswith('quartz.'):
                ...
                if str(camera_id) == str(self.cam_uuid_re) :
    ...
    def capture_images(self, config=None):
        ...
        for camera in self.nest_camera_array:
            ...
            # or not.
            if not config.path:
                camera_path = os.path.join(self.local_path,camera)
            else:
                camera_path = os.path.join(config.path,camera)
    ...
    def perform_capture(self, config=None, camera=None, camera_path=''):
        ...
        while self.is_capturing:
            ...
            try:
                ...
                with open(camera_path + "/" + 'img'+ str(count) + '.jpg', 'wb') as image_file:
                ...
                '''if config.produce_video:
                    ...
                    if camera_buffer_size < self.nest_camera_buffer_threshold:
                        camera_buffer[camera].append(str(count))
                    else:
